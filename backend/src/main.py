...
from src.agents.orchestrator import process_chat
...
from src.document_validation import validate_image
from src.events.event_bus import publish, subscribe
from src.notifications.status_notification import handle_status_change
...
def chat(pergunta: str = Body(..., embed=True)):
    ...
    resultado = process_chat(pergunta, status_data)
    return resultado
...
def get_chat_history():
    ...
    cursor.execute("""
        SELECT id, session_id, user_question, intent, entities, ai_response, created_at
